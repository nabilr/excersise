...
#ifndef NET_QUIC_CONGESTION_CONTROL_TCP_VERUS_SENDER_BASE_H_
#define NET_QUIC_CONGESTION_CONTROL_TCP_VERUS_SENDER_BASE_H_
...
namespace test {
class TcpVerusSenderBasePeer;
}  // namespace test

class NET_EXPORT_PRIVATE TcpVerusSenderBase : public SendAlgorithmInterface {
 public:
  // Reno option and max_tcp_congestion_window are provided for testing.
  TcpVerusSenderBase(const QuicClock* clock,
                     const RttStats* rtt_stats,
                     bool reno,
                     QuicConnectionStats* stats);
  ~TcpVerusSenderBase() override;
 ...
 protected:
  ...
  void SetCongestionWindowInPackets(
 ...
 private:
  friend class test::TcpVerusSenderBasePeer;
 ...
 protected:
  ...
  bool is_timer_expires;
  base::RepeatingTimer timer_;
  //base::TimeDelta now_;

  QuicClock  clock_;
  QuicTime next_expiry_time_;

  

 private:

  void set_timer_expires (bool flag);

  DISALLOW_COPY_AND_ASSIGN(TcpVerusSenderBase);
};
